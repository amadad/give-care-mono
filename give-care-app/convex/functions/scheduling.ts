...
async function canSendProactiveMessage(
  ...
  if (user.lastProactiveMessageAt && user.lastProactiveMessageAt > oneDayAgo) {
    console.log(`[Dedup] Skipping ${userId} - already sent proactive message today`);
    return false;
  }

  // Check if user contacted us today (reactive message)
  if (user.lastContactAt && user.lastContactAt > oneDayAgo) {
    console.log(`[Dedup] Skipping ${userId} - user already active today`);
    return false;
  }
...
export const sendTieredWellnessCheckins = internalAction({
  handler: async (ctx) => {
    console.log('[Cron] sendTieredWellnessCheckins starting...');

    const now = Date.now();
    let sentCount = 0;

    // Crisis users (first 7 days) - Daily check-ins
    const crisisRecentUsers = await ctx.runQuery(
      internal.functions.users.getEligibleForCrisisDaily
    );
    ...
    const moderateUsers = await ctx.runQuery(
      internal.functions.users.getEligibleForModerateCheckin
    );
    ...
    console.log(`[Cron] sendTieredWellnessCheckins complete: sent ${sentCount} check-ins`);
...
export const reactivateDormantUsers = internalAction({
  handler: async (ctx) => {
    console.log('[Cron] reactivateDormantUsers starting...');
    ...
    const dormantUsers = await ctx.runQuery(
      internal.functions.users.getDormantAtMilestones
    );
    ...
    for (const user of dormantUsers) {
      ...
      // Day 31+: Mark as churned (stop messaging)
        ...
        console.log(`[Churn] Marked user ${user._id} as churned after ${Math.floor(daysSinceContact)} days inactive`);
        churnedCount++;
      }
    }

    console.log(`[Cron] reactivateDormantUsers complete: sent ${sentCount}, churned ${churnedCount}`);
...
export const generateWeeklyReport = internalAction({
  handler: async (ctx) => {
    console.log('[Cron] generateWeeklyReport starting...');
    ...
    console.log('[Weekly Report]', JSON.stringify(stats, null, 2));

    // TODO: Send email or post to Slack
    // await sendAdminEmail(stats);
    // await postToSlack(stats);

    console.log('[Cron] generateWeeklyReport complete');
    return stats;
  },
});
...
export const scheduleMessage = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(`[Schedule] Queued ${args.type} for ${args.userId} in ${args.delayMs}ms`);
...
export const sendScheduledMessage = internalAction({
  ...
  handler: async (ctx, args) => {
    console.log(`[Scheduled] Executing ${args.type} for ${args.userId}`);

    // Get user
    const user = await ctx.runQuery(internal.functions.users.getUser, { userId: args.userId });
    if (!user) {
      console.error(`[Scheduled] User ${args.userId} not found`);
...
export const scheduleCrisisFollowups = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(`[Crisis] Scheduled 7-stage follow-up for ${userId}`);
...
export const checkOnboardingAndNudge = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (isComplete) {
      console.log(`[Onboarding] User ${userId} already completed profile - skipping nudge ${nudgeStage}`);
    ...
    let message: string;
    if (nudgeStage === 1) {
      message = "Hey! Have a moment to finish setting up your profile? It helps me support you better 💙";
    } else {
      message = "Just checking in - we'd love to support you. Finishing your profile takes 2 minutes 💙";
    }

    await sendProactiveMessage(ctx, userId, user.phoneNumber, message, `onboarding_nudge_${nudgeStage}`);
