...
export const getConversationMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Agent breakdown
    const agentCounts = assistantMessages.reduce((acc, m) => {
    ...
    const tierCounts = assistantMessages.reduce((acc, m) => {
    ...
    // Tool usage stats
    const toolUsage = assistantMessages.reduce((acc, m) => {
...
export const getServiceTierMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const tierMetrics = messages.reduce((acc, m) => {
      const tier = m.serviceTier || 'unknown';
      if (!acc[tier]) {
        acc[tier] = {
          count: 0,
