/**
 * Admin-facing queries for dashboards and system health.
 */
...
const extractMetadata = (session: Doc<'sessions'> | null) =>
  ((session?.metadata as Record<string, unknown>) ?? {});

const extractProfile = (metadata: Record<string, unknown>) =>
...
export const getMetrics = query({
  args: {},
  handler: async (ctx) => {
    const latestDaily = await ctx.db.query('metrics_daily').withIndex('by_date').order('desc').first();
    const subscriptionMetrics = await ctx.db.query('metrics_subscriptions').first();
    ...
    return {
      ...
      crisisAlerts: metrics.crisisAlerts,
      avgBurnoutScore: metrics.avgBurnoutScore,
      p95ResponseTime: metrics.p95ResponseLatencyMs,
      subscriptionBreakdown,
      activeSubscriptions: subMetrics.active,
      alertsLast24h: metrics.crisisAlerts,
      avgLatencyMs: metrics.avgResponseLatencyMs,
    };
  },
});
...
export const getAllUsers = query({
  ...
  handler: async (ctx, args) => {
    ...
    const users = await ctx.db.query('users').order('desc').take(limit * 3);
...
export const getUserDetails = query({
  ...
  handler: async (ctx, { userId }) => {
    ...
    const scores = await ctx.db
      ...
      .order('desc')
      .take(30);
    const wellnessHistory = scores
      .map((score) => ({ recordedAt: score._creationTime, score: score.composite }))
      .reverse();
    ...
    const history = assessments.map((assessment) => ({
      ...
      recordedAt: assessment._creationTime,
      score: scoreByAssessment.get(assessment._id)?.composite ?? null,
      band: scoreByAssessment.get(assessment._id)?.band ?? null,
    }));
...
export const getSystemHealth = query({
  ...
  handler: async (ctx) => {
    ...
    const tokensToday = runs.reduce(
      (sum, run) => sum + run.budgetResult.usedInputTokens + run.budgetResult.usedOutputTokens,
      0
    );
    ...
    return {
      ...
      database: {
        queryLatency,
        connectionPoolActive: recentSessions.length,
...
export const getCrisisAlerts = query({
  ...
  handler: async (ctx) => {
    ...
    for (const score of scores) {
      ...
      crisisUsers.push({
        ...summary,
        burnoutScore: score.composite,
        burnoutBand: score.band,
        pressureZones: summary.pressureZones,
      });
      seen.add(key);
...
export const getBillingEvents = query({
  args: {},
  handler: async (ctx) => {
    const events = await ctx.db.query('billing_events').order('desc').take(20);
