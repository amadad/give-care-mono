/**
 * Watchers domain - background engagement checks that raise alerts.
 */
...
export const runEngagementChecks = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const user of users) {
      ...
      const crisisHits = recentMessages.filter(
        (message) =>
          message.direction === 'inbound' &&
          CRISIS_TERMS.some((term) => message.text.toLowerCase().includes(term))
      );
    ...
    console.info(
      `[watchers] Completed engagement checks in ${duration}ms, processed ${users.length} users, next cursor: ${newCursor ?? 'reset'}`
    );
