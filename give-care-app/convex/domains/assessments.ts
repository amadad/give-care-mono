/**
 * Assessment sessions, question banks, and scoring logic.
 */
...
const getQuestions = (definitionId: string) => {
  const questions = QUESTIONS[definitionId as keyof typeof QUESTIONS];
  if (!questions) {
...
const scoreAnswers = (definitionId: string, answers: Array<{ questionId: string; value: number }>) => {
  ...
  if (definitionId === 'bsfc_v1') {
    const band = total >= 24 ? 'high' : total >= 16 ? 'medium' : 'low';
    const zoneScores = answers.reduce<Record<string, number>>((acc, answer) => {
      const question = QUESTIONS.bsfc_v1.find((q) => q.id === answer.questionId);
      if (question?.zone) {
        acc[question.zone] = (acc[question.zone] ?? 0) + answer.value;
      }
      return acc;
    }, {});
    ...
    return {
      total,
      band,
      explanation: `BSFC score ${total}/30 indicates ${band} burden.`,
      pressureZones,
    };
  }

  if (definitionId === 'ema_v1') {
    const stressQuestion = answers.find((answer) => answer.questionId === 'stress');
    const stressLevel = stressQuestion ? stressQuestion.value : total;
  ...
  if (definitionId === 'sdoh_v1') {
    ...
    answers.forEach((answer) => {
      ...
      if (qId.startsWith('food')) zoneScores.food += answer.value;
        ...
        qId.startsWith('financial') ||
        qId.includes('insurance') ||
        qId.includes('medical_cost')
      )
        zoneScores.financial += answer.value;
    });
