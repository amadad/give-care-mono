/**
 * Analytics domain powering dashboards and reporting.
 */
...
export const getBurnoutDistribution = query({
  args: {},
  handler: async (ctx) => {
    const distribution = await ctx.db.query('metrics_burnout_distribution').collect();
...
export const getUserJourneyFunnel = query({
  args: {},
  handler: async (ctx) => {
    const funnel = await ctx.db.query('metrics_journey_funnel').first();
...
export const getQualityMetrics = query({
  ...
  handler: async (ctx, { days }) => {
    ...
    const averageComposite = scores.length
      ? scores.reduce((sum, score) => sum + score.composite, 0) / scores.length
      : 60;
...
export const getSummaryPerformance = query({
  ...
  handler: async (ctx) => {
    ...
    const versionStats = new Map<string, { count: number; tokens: number; cost: number }>();
    let totalTokens = 0;
    let totalCost = 0;
    for (const run of runs) {
