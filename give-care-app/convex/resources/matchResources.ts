...
export const matchResourcesForUser = query({
  ...
  handler: async (ctx, { userId, limit = 10 }): Promise<ScoredResource[]> => {
    ...
    const validResources = scoredResources.filter(
      (r): r is ScoredResource => r !== null
    );
...
function calculateZoneScore(
  ...
  const overlap = userZones.filter(zone =>
    program.pressureZones.includes(zone)
  ).length;
...
function calculateQualityScore(resource: Doc<"resources">): number {
  ...
  const rbiScore = resource.scoreRbi !== undefined && resource.scoreRbi !== null
    ? resource.scoreRbi
  ...
  const verificationBoost =
    verificationBoosts[resource.verificationStatus] || 0.3;
...
function calculateFreshnessScore(resource: Doc<"resources">): number {
  ...
  const daysSinceVerification =
    (Date.now() - resource.lastVerifiedDate) / (24 * 60 * 60 * 1000);
