...
export const importWithLogging = internalMutation({
  ...
  handler: async (ctx, { fileContent, dryRun = false }) => {
    ...
    const isJunk = titleLower.includes('emergency operations') || titleLower.includes('environmental');
    ...
    if (dryRun) {
      ...
      log(`Total: ${estimatedValid} resources × 5 tables = ~${estimatedValid * 5} database records\n`);
    } else {
      ...
      try {
        ...
        log(`📊 Created 5 records (1 provider, 1 facility, 1 program, 1 service area, 1 resource)\n`);
