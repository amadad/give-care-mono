...
/**
 * Aggregate daily metrics - runs once per day via cron
 * Replaces expensive .collect() calls in admin queries
 */
...
export const computeDailyMetrics = internalMutation({
  ...
  handler: async (ctx, { date }) => {
    ...
    // Total users (indexed scan)
    const allUsers = await ctx.db.query('users').collect();
    const totalUsers = allUsers.length;

    // New users today (indexed by creation time)
    const newUsersToday = allUsers.filter(u => u._creationTime >= dayAgo).length;

    // Active users (via sessions)
    const recentSessions = await ctx.db
      .query('sessions')
      .filter((q) => q.gte(q.field('lastSeen'), dayAgo))
      .collect();
    const activeUsers = new Set(recentSessions.map(s => s.userId)).size;

    // Message counts (indexed by creation time)
    ...
    // Average burnout score (latest per user)
    ...
    // Response latency (from agent runs)
...
export const computeSubscriptionMetrics = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const metrics = {
      ...
      pastDue,
      canceled,
      free: total - subscriptions.length,
